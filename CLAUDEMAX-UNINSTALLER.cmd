...
chcp 65001 >nul
title CLAUDEMAX — Desinstalador
cd /d "%~dp0"

echo.
echo   CLAUDEMAX — Desinstalador
echo   ============================================================
echo   Este asistente desinstala CLAUDEMAX de tu equipo: te muestra
echo   primero que se elimina y que se conserva, y solo actua si
echo   confirmas explicitamente. No hace falta saber nada tecnico.
echo.
...
echo.
echo   Arrancando el asistente de desinstalacion...
echo.
"%NODE_BIN%" "%~dp0bin\wizard\wizard.mjs" --uninstall %*
set "WIZARD_EXIT=%ERRORLEVEL%"

echo.
if "%WIZARD_EXIT%"=="0" (
    echo   [OK] El proceso termino sin errores.
) else (
    echo   [ERROR] La desinstalacion no se completo — revisa los mensajes de arriba.
)
pause
exit /b %WIZARD_EXIT%
...
:fin_error
echo.
echo   [ERROR] No se pudo preparar el equipo para desinstalar CLAUDEMAX.
pause
exit /b 1
...
echo   CLAUDEMAX necesita Node.js version 18 o superior para desinstalar.
echo.
...
if errorlevel 1 (
    ...
    echo   Vuelve a ejecutar este desinstalador cuando termines.
    exit /b 1
)
...
echo   Cierra esta ventana y vuelve a ejecutar CLAUDEMAX-UNINSTALLER.cmd — el PATH ya
