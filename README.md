...
4. Selección de componentes (los mismos once de la tabla de arriba).
...
| `--only <id>` | Solo un componente. Repetible. ids: `rtk`, `figma`, `ui-ux`, `impeccable`, `dev-skills`, `rag`, `graphify`, `ponytail`, `cyber-neo`, `parsers`, `rules`. |
...
3. Prueba los comandos:
   ...
   - `/impeccable init` — primer paso de Impeccable en un proyecto: escribe `PRODUCT.md` con el contexto de producto (para código ya existente, `/impeccable document` genera `DESIGN.md` a partir de lo que hay). Después, la orden que toque: `/impeccable shape` antes de codificar una pantalla, `/impeccable critique` o `/impeccable audit` para revisarla, `/impeccable polish` antes de publicar, `/impeccable live` para iterar variantes en la app corriendo. Acepta objetivo: `/impeccable audit el header`.
   ...
   - Edita un `.tsx`/`.css`/`.vue`/`.html` — el hook `ui-audit.mjs` revisa el resultado y avisa por `system-reminder` si detecta anti-patrones de UI (gradient text de relleno, nombres placeholder, tarjetas idénticas, etc.). No bloquea nada; desactívalo con `CLAUDEMAX_UI_AUDIT=0` si te resulta ruidoso. El hook de Impeccable escucha el mismo evento con sus 59 detectores: ninguno de los dos bloquea, pero verás dos avisos — si sobra uno, el nuestro es el que se apaga con esa variable.
