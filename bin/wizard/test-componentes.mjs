...
// de bin/install.sh con una expresión regular en JS. Esta prueba verifica esa extracción de
...
//
// Por seguridad no se hace `source bin/install.sh` completo: ese script no está escrito como
...
if (!lineaMatch) {
    console.error("FALLO: no se encontró la línea 'ALL_COMPONENTS=(...)' en bin/install.sh.");
    process.exit(1);
}
...
if (!bash) {
    console.warn(
        "AVISO: no se encontró bash — no se puede hacer la verificación cruzada real contra bin/install.sh."
    );
    console.warn("Se omite la comprobación estricta (no se marca como fallo solo por falta de bash).");
    process.exit(0);
}

// Evalúa SOLO esa línea (no todo bin/install.sh) en un bash aislado y de usar-y-tirar: sin
