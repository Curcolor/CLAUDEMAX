...
# -*- coding: utf-8 -*-
"""
UI/UX Pro Max Core - motor de búsqueda BM25 para guías de estilo UI/UX
"""
...
# Columnas comunes para todos los stacks
_STACK_COLS = {
...
class BM25:
    ...
    def tokenize(self, text):
        """Minúsculas, split, quitar puntuación, filtrar palabras cortas"""
        text = re.sub(r'[^\w\s]', ' ', str(text).lower())
        return [w for w in text.split() if len(w) > 2]

    def fit(self, documents):
        """Construye el índice BM25 a partir de los documentos"""
    ...
    def score(self, query):
        """Puntúa todos los documentos contra la query"""
...
def _load_csv(filepath):
    """Carga el CSV y devuelve una lista de dicts"""
    with open(filepath, 'r', encoding='utf-8') as f:
        return list(csv.DictReader(f))


def _search_csv(filepath, search_cols, output_cols, query, max_results):
    """Función de búsqueda central usando BM25"""
    if not filepath.exists():
        return []

    data = _load_csv(filepath)

    # Construye los documentos a partir de las columnas de búsqueda
    ...
    # Obtiene los mejores resultados con score > 0
    results = []
...
def detect_domain(query):
    """Auto-detecta el dominio más relevante a partir de la query"""
...
def search(query, domain=None, max_results=MAX_RESULTS):
    """Función de búsqueda principal con auto-detección de dominio"""
    if domain is None:
...
def search_stack(query, stack, max_results=MAX_RESULTS):
    """Busca guías específicas de un stack"""
